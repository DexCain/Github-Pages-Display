...
// This function checks to ensure that the user has clicked the correct action button
...
async function prepTraining() {
    ...
    // We give the patient a random age, so that the situation is different each time
...
// This function is for rescue breaths when the airway is obstructed
async function cprBreathsObstructed(firstTime, clearAirway) {
    ...
    // Wait until they click on the patient to give a breath
    while(true){
        event = await waitForEvent(window, 'click');
    ...
    // Changing choice based on if we want the airway to be clear now
    if(clearAirway){
    ...
    // Wait until they give the second breath
    while(true){
        event = await waitForEvent(window, 'click');
        if(event.target == image){
            gameWarning.style.display = 'none';
...
// This is a function performed before the obstructed airway, the user is checking the patients mouth for dislodged food
...
// This is a async function that sets off the event where the patient wakes up 10 seconds into supporting the persons life
...
// This is a function that waits until the user clicks on the survey the scene button to ensure their own saftey
...
// This is a function that waits until the user checks for the patients conciousness
...
// This is a function that waits until the user checks the pulse
async function checkThePulse(cond) {
    var checkButton;
    // Depending on the age of the individal, we much check a different artery
    if(age === "infant"){
    ...
    // Depending on the event, we have different prompts to give the user
    if (cond === "cpr"){
...
// When we are ending with someone awake, or with a pulse and breathing, we wait until the uset puts them in the recovery position
...
// This function starts the CPR and tracks its progress by giving the user updates as to their timing and quality of cpr
async function startCPR(stopReason, obstructedAirway) {
    ...
    // If we have a stop reason, then we will stop during this round of CPR and the function should end during this method
    // Which is why we call these async functions synchrounously
    if(stopReason === 'ems'){
        emsArrivesDuringCpr();
        inFuncStop = false;
    }
    ...
    // Starting the cpr, every 100ms, we check the avg cpr timing and ensure it falls within an acceptable range
    resetCPRTimes();
    while( count !== 30){
...
// This checks for cpr clicks, and manages the time inbetween clicks and the last three clicks average
...
// This is a function that tracks the breaths during cpr, and ensures that the user gives breaths (even through foam)
async function cprBreathsSuccessful(foam) {
    var event = true;

    // Wait until they equip the sealeasy
    await waitForCorrectButton('sealeasy', 'We completed our 30 compressions, what is next?', 'What do we do after 30 compressions for CPR');
    ...
    // Wait until they place the mask on the GID
    if(gameHintState){
        hint.innerHTML = "Place the mask on the GID"
    }
    while(true){
    ...
    // We can try to trip them up by telling them their is foam coming from the mouth which they should breath through
    if(foam){
...
// This is a function that starts AR
async function startAR(cond){
    ...
    // If the person is an adult, then they have different AR counts than an infant or child
    // Thus we need to change the low and high avg (which give 450ms of extra room)
    if(age === "adult"){
        finalCount = 20;
        lowAvg = 5550;
        highAvg = 6450;
    ...
    // Reseting the AR time to prepare to perform AR
    resetARTimes();
    ...
    // IF we want to trip up the user and give them a patient vomiting
    var vomit = cond === 'vomit';

    // Every 100ms, we check the condition for the while statement
    while(arCount !== finalCount){
        ...
        // Checking if the AR avg is correct
        if(arAvg < lowAvg){
...
// Reset AR time for another round of AR
...
// This is another function that ends CPR, where the EMS arrives during cpr, waits until the user takes their hands off the patient and hands care to ems
...
// This is the reactive left-side menu, and everytime it is clicked, it resets the dropdowns and opens the correct drop-down
...
// This allows the custom cursor to follo the cursor
const updateCursorPosition = (event) => {
  customCursor.style.top = `${event.clientY}px`;
  customCursor.style.left = `${event.clientX}px`;
}

// Every time the mouse if moved, we update the custom cursors position
...
// This is a function for the first menu, that removes the selected value from all the train type buttons
...
// anytime one of the training buttons is clicked, we need to reset the training buttons and select the one selected.
trainingButtons.addEventListener("click", (event) => {

    var element = event.target;
    // Checking if the span within the Div was clicked an ensuring we reference the DIV
    if(element.tagName === "SPAN"){
        element = element.parentElement;
    }
    // Ensuring that clicking within the trainingbuttons container, but not on a button doesn't unselect all buttons
    if(element.id !== "training-selection-buttons"){
...
// This is similar to a method above, where is just clears all the choices.
...
// Adds the selected div to teh specific training type we want
trainingChoices.addEventListener("click", (event) => {
    // ensuring that clicking on the parent container of the buttons doesn't affect the fucntionality
    if(event.target.id !== "training-choice"){
...
// When the start button is clicked, we need to check which divs are selected to choose the correct training
startButton.addEventListener("click", () => {
 
    gameWarning.style.display = "none";

    // Depending on the type of training they want to complete, we have to set up the menu accordingly
    ...
    // Depending on their choice of training, we choose the correct training method
    if(cprTraining.classList.contains("selected-div")){
        ...
        // If they choose random, then we randomly select one of the trainings
    ...
    // hide the selection menu, and display the game
    cprContainer.style.display = "none";
    arContainer.style.display = "none";
